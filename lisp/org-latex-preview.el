...
:page-width  The width of the LaTeX document fragments are compiled in. Either:
...
(defcustom org-latex-preview-auto-command-blacklist nil
...
(defun org-latex-preview--update-overlay (ov path-info)
  ...
  (let* ((zoom (or (plist-get org-latex-preview-appearance-options :zoom) 1.0))
    ...
    (when errors
        (overlay-put
         ov 'help-echo
         (if (bound-and-true-p toolip-mode)
             errors
...
It is either a boolean value, or a list of context symbols within
which live previews should be shown.
...
;;
;; Currently not used.  Adding this function to
;; `org-latex-preview-process-finish-functions' is potentially more
;; efficient than adding a function to
;; `org-latex-preview-update-overlay-functions' on LaTeX runs with
;; many fragments, but for some reason the image display fails to
;; update until a full redisplay/the user runs a command.  This needs
;; to be investigated.
