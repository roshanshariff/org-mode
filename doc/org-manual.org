...
cache can also be manually cleared if required.

- (~org-latex-preview-clear-cache~) ::
  #+findex: org-latex-preview-clear-cache

  Clear the LaTeX preview cache.
  ...
  With a prefix argument, clear the entire LaTeX preview cache and
  delete all cached LaTeX preview images.
...
A small preamble is recommended for real-time LaTeX previews.  To help
with this you can specify header content to be used for LaTeX exports
but not previews via the =#+LATEX_HEADER_EXTRA= keyword, (see [[*LaTeX
...
#+vindex: org-latex-preview-process-precompiled
Precompilation is enabled by default, and you can disable it by
setting ~org-latex-preview-process-precompiled~.  Currently only
...
**** Automatic and live LaTeX previews

Org mode provides two levels of automatic LaTeX preview generation
with increasing degrees of immediate feedback:

- Automatic previews: It can hide and reveal preview images as the
  cursor moves into or out of them, and automatically preview LaTeX
  fragments after you finish typing them and exit the fragment.

- Live previews: In addition to the above, it can continuously preview
  LaTeX fragments as you type, giving you near immediate feedback
...
- (~org-latex-preview-auto-mode~) ::
  ...
  Moving the cursor into or out of a preview image will cause the
  image to be hidden or revealed respectively.

#+vindex: org-latex-preview-auto-track-inserts
~org-latex-preview-auto-mode~ will always hide and show preview images
...
controlled by the option ~org-latex-preview-auto-track-inserts~.

#+vindex: org-latex-preview-auto-ignored-environments
You can specify a list of LaTeX environments that should
not be automatically previewed by ~org-latex-preview-auto-mode~ when
they are inserted into the buffer.  This is controlled by
~org-latex-preview-auto-ignored-environments~.

#+vindex: org-latex-preview-auto-ignored-commands
You can also specify Emacs commands that should explicitly avoid
toggling the preview image with
~org-latex-preview-auto-ignored-commands~.  For instance, adding the
...
When ~org-latex-preview-auto-mode~ is active, the hooks
...
Continuous, live previews can be enabled by setting
~org-latex-preview-live~ and turning on ~org-latex-preview-auto-mode~.
~org-latex-preview-live~ can be configured to apply separately to
...
#+vindex: org-latex-preview-live-display-type
...
~org-latex-preview-live-display-type~ and turn on ~eldoc-mode~.
