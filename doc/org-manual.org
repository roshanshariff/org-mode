...
- {{{kbd(M-x org-capture)}}} (~org-capture~) ::
  ...
  selection.  It inserts the template into the target file and switch
  to an indirect buffer narrowed to this new node.  You may then
...
Previewing or exporting images of LaTeX fragments also requires the
preview.sty package or style file[fn:40] to be available.
...
You can set this globally by customizing
~org-startup-with-latex-preview~.

Org mode caches LaTeX fragment preview images so that repeated calls
to ~org-latex-preview~ for the same fragment can reuse them.  The
cache can also be manually cleared if required (see [[*Customizing the
LaTeX preview process]]).

Errors encountered when compiling preview fragments are captured and
indicated in the display of the preview image.  Depending on the type
of processing, this indication can take the form of a red foreground
or a fringe marker.  Mousing over the image will show the error(s) in
a tooltip, and they can be displayed in the echo area by calling
~display-local-help~ when the cursor is on the image or LaTeX fragment.

Normally, LaTeX preview images cover the underlying LaTeX text.
Editing and updating LaTeX fragments thus requires clearing the
previews first.  To speed up the edit/preview cycle, Org also provides
optional /automatic/ preview features with more immediate feedback:

- Automatic previews: Hide and reveal preview images when the cursor
  moves into or out of them, and automatically preview LaTeX fragments
  after the cursor exits the fragment.
...
To enable automatic previews, turn on the minor mode
~org-latex-preview-mode~.

- (~org-latex-preview-mode~) ::
  #+findex: org-latex-preview-mode

  When this mode is turned on, moving the cursor into a previewed
  LaTeX fragment will reveal the Org markup and hide the image.
  Moving the cursor out of the fragment will cause the (possibly
  updated) preview image to be displayed again.

  If ~org-latex-preview-mode-track-inserts~ is non-nil, previews for
  LaTeX fragments will be auto-generated as they are typed or
  otherwise inserted into the buffer.

  Note that insertion tracking does not work across indirect buffers.
  LaTeX preview images are generated only for the current buffer, and
  not any indirect clones, or for the base buffer if the current
  buffer is a clone.  This is currently a limitation of the LaTeX
  preview system.

  #+vindex: org-latex-preview-mode-display-live
  ...
  separately to LaTeX fragments and LaTeX environments.  Additionally,
  it can also be configured to preview LaTeX when using
  ~org-edit-special~.

**** Customizing live LaTeX previews

#+vindex: org-latex-preview-mode-track-inserts
~org-latex-preview-mode~ will always hide and show preview images
already present in the buffer as the cursor moves into and out of
LaTeX fragments.  However, you can customize whether it should also
generate new previews as the buffer contents are changed.  This is
controlled by the option ~org-latex-preview-mode-track-inserts~.

#+vindex: org-latex-preview-mode-ignored-environments
You can specify a list of LaTeX environments that should not be
automatically previewed by ~org-latex-preview-mode~ when they are
inserted into the buffer.  This is controlled by
~org-latex-preview-mode-ignored-environments~.  By default, "figure"
environments are ignored by ~org-latex-preview~.[fn::
~org-latex-preview~ cannot consistently preview figure environments.]

#+vindex: org-latex-preview-mode-ignored-commands
You can also specify Emacs commands that should explicitly avoid
toggling the preview image with
~org-latex-preview-mode-ignored-commands~.  For instance, adding the
~next-line~ and ~previous-line~ commands to this list will ensure that
regular Emacs navigation does not cause the preview image for a
fragment to be toggled, which can be distracting.

#+vindex: org-latex-preview-update-delay
...
setting ~org-latex-preview-mode-update-delay~.  Note that values of
0.1 or lower can cause high CPU utilization.

#+vindex: org-latex-preview-overlay-open-functions
#+vindex: org-latex-preview-overlay-close-functions
When ~org-latex-preview-mode~ is active, the hooks
~org-latex-preview-overlay-open-functions~ and
~org-latex-preview-overlay-close-functions~ run when the cursor moves
into or out of a preview overlay, respectively.  Hook functions are
called with one argument, the overlay being "opened" or "closed".
These hooks can be used to modify the overlay or image properties.

#+vindex: org-latex-preview-mode-display-type
Finally, when editing a fragment live previews can be displayed in a
few different locations.  The default setting displays them next to
the LaTeX element when editing inline fragments, and under the LaTeX
element when editing LaTeX environments.  Alternatively, the preview
can be displayed via Eldoc in a location determined by
~eldoc-display-functions~.  To set this customize
~org-latex-preview-mode-display-type~ and turn on ~eldoc-mode~.

**** Customizing the LaTeX preview process

The LaTeX preview process pipeline works as follows:

#+cindex: @LaTeX{} fragments, precompilation
1. LaTeX fragments in the region being previewed are collected.
2. The LaTeX preamble for the buffer is constructed.  If the LaTeX
   compiler in use (see next) supports precompilation, an object dump
   is created to drastically speed up subsequent previews in the
   buffer.
3. They are compiled to dvi format using ~org-latex-compiler~, which
   can be =pdflatex=, =xelatex= or =lualatex= (see [[*LaTeX/PDF export
   commands]]).
4. The dvi file is converted to an image placed over the text of the
   LaTeX fragments in Org buffers.  These images can be PNGs (when
   using the =dvipng= or =convert= programs) or SVGs (when using
   =dvisvgm=).  This image is then cached for the future.
5. When previewing an unmodified LaTeX fragment for a second time, the
   image is placed directly from the cache.

Accordingly, each step of this pipeline can be independently customized.

#+vindex: org-latex-preview-process-precompile
Precompilation is enabled by default, and you can disable it by
setting ~org-latex-preview-process-precompile~.  Currently only
=pdflatex= supports precompilation consistently, and it is
automatically disabled when using =xelatex= or =lualatex=.  It is also
disabled buffer-locally if the precompilation attempt fails for some
reason.  In this case you can re-enable it by reverting the buffer or
running ~org-mode-restart~.

You can customize the LaTeX engine used to process the fragments by
setting ~org-latex-compiler~, (see [[*LaTeX/PDF export commands]]).
=pdflatex= with precompilation can produce previews that update the
fastest, close to real-time.  =lualatex= has the best font and unicode
support but previewing will be much slower.

#+vindex: org-latex-preview-process-default
To choose the program used for generating preview images, you can set
the option ~org-latex-preview-process-default~ to one of the symbols
=dvipng=, =dvisvgm= or =imagemagick=.  =dvisvgm=, the default, offers
the best quality images that display well on HiDPI screens, as well as
features like Emacs theme awareness.  =dvipng= previews are more basic
but are generated the fastest.  The =imagemagick= route is supported
but slow, and should be avoided if possible.

#+vindex: org-latex-preview-process-alist
Details of the process chain, such as the command line arguments used
to call =pdflatex= and =dvisvgm=, can be changed by modifying
~org-latex-preview-process-alist~.  It is not recommended to do
this unless you have very specific needs.

#+vindex: org-latex-preview-cache
#+vindex: org-latex-preview-persist-expiry
You can control where LaTeX previews are cached by customizing
~org-latex-preview-cache~.  By default, Org stores them in
~org-persist-directory~ with an expiry determined by
~org-latex-preview-persist-expiry~.  It can also be set to a string
naming an absolute or relative path, or to the symbol =temp= to not
cache images beyond the current Emacs session.

#+findex: org-latex-preview-clear-cache
The LaTeX preview cache can be manually cleared (globally or
buffer-locally) by calling ~org-latex-preview-clear-cache~.

#+vindex: org-latex-preview-process-active-indicator
When a preview process is running, a visual processing indicator is
placed on the corresponding LaTeX fragments.  This defaults to a
triangle in the fringe to the left, but can be customized by setting
~org-latex-preview-process-active-indicator~.

#+vindex: org-latex-preview-process-finish-functions
#+vindex: org-latex-preview-overlay-update-functions
Two hooks may be used to perform additional processing when generating
or updating previews for fragments.  The hook
~org-latex-preview-process-finish-functions~ may be used to perform
additional processing at the end of a preview creation process chain.
Each hook function is called with three process-related arguments: the
process exit code, the process buffer and a plist of process data.
~org-latex-preview-overlay-update-functions~ runs when the preview
image for an overlay changes.  It can be used to modify the overlay
appropriately, for example to center the image in the buffer.  Hook
functions are called with one argument, the overlay being updated.

**** Customizing the LaTeX preview preamble
...
~org-latex-packages-alist~ respectively.  See ~org-latex-classes~ for
a detailed description of the placeholders.  When previewing, this
preamble will be combined with keywords specified in the document via

#+texinfo: @noindent
: #+LATEX_HEADER:

A small preamble is recommended if you are using real-time (live)
LaTeX previews.  To help with this you can specify header content to
be used for LaTeX exports but not previews via the
=#+LATEX_HEADER_EXTRA= keyword, (see [[*LaTeX specific export
settings]]).

**** Customizing the LaTeX preview appearance

By default, LaTeX previews are colored, sized and aligned to fit in
correctly with the surrounding text.  If you are using SVG previews
(the default), they will also change colors automatically when you
change faces or Emacs themes.
...
specify the following aspects of the appearance of LaTeX previews:

- Foreground and background: by default the foreground matches the
  color of the surrounding text and the background is transparent.
  The ~:foreground~ and ~:background~ properties can be used to change
  this.
- Zoom and scale: The ~:zoom~ property controls the relative size of
  the preview as displayed in the buffer, and ~:scale~ controls the
  size of the generated image.  ~:scale~ has no meaning for SVG
  images.
- Page width: The ~:page-width~ property determines the width of the
  preview image (including whitespace padding) of LaTeX environments.
...
behavior is controlled by the user option
...
for the example to look like the fontified Emacs buffer[fn:41].  This
is done with the code block, where you also need to specify the name
of the major mode that should be used to fontify the example[fn:42],
...
...
[fn:51] The link is: https://orgmode.org
#+end_example
...
contents[fn:44].  If you need to exclude an unnumbered headline,
...
~org-link-search~[fn:45] (see [[*Search Options in File Links]]).  The
...
#+texinfo: @noindent
which can be referenced using ={{{name(arg1, arg2)}}}=[fn:46].  For
example
...
- Org exports a Beamer frame's objects as block environments.  Org can
  ...
  is set[fn:47].  For valid values see
...
with MathJax Components]].][fn:48].  Some MathJax options can be
...
Alternatively, LaTeX fragments can be processed into images that will
be inserted into the browser page.  This method requires that the
dvipng program, dvisvgm or ImageMagick suite be available on your
system.  Before the availability of MathJax, this was the default
method for Org files. You can get this processing with
...
The images can be inserted as links or as inline data for a more
portable HTML document.  When using =dvisvgm=, SVG images can
additionally be embedded as DOM objects in the page.  This method has
the advantage that it respects theme changes and some CSS
specifications.

In all cases, the images are scaled and aligned to fit in with
surrounding text, respecting baselines and ascent.  The LaTeX source
for an image is available as its alt-text in the HTML document.

#+vindex: org-html-latex-image-options
The apperance and inclusion method of LaTeX preview images in HTML
exports is governed by the option ~org-html-latex-image-options~.
In particular, the =:image-dir= key controls the absolute or relative
path where LaTeX preview images are stored, and the =:inline= key
can be used to specify image types (=png= and/or =svg=) to inline or
embed instead.
...
- MathML ::
  ...
  #+vindex: org-mathml-convert-command
  #+vindex: org-mathml-converter-jar-file
  You can specify the LaTeX-to-MathML converter by customizing the
  variables ~org-mathml-convert-command~ and
  ...
  #+begin_src emacs-lisp
  (setq org-mathml-convert-command
...
By calling a named code block[fn:50] from an Org mode buffer or
...
- =value= ::
  ...
  Default for most Babel libraries[fn:50].  Functional mode.  Org
...
Emacs[fn:53].  With Org's support, you have to indent all lines to
...
a server[fn:54] to interact with Emacs.  Pass its location through
...
Orgtbl mode inserts a radio table skeleton[fn:56] with the command
...
[fn:40] See https://ctan.org/tex-archive/macros/latex/contrib/preview
for details on preview.sty.  This package is included by default when
installing all commonly used TeX distributions.

[fn:41] This works automatically for the HTML backend (it requires
...
[fn:42] Source code in code blocks may also be evaluated either
interactively or on export.  See [[*Working with Source Code]] for more
information on evaluating code blocks.

[fn:43] For export to LaTeX format---or LaTeX-related formats such as
Beamer---, the =org-latex-package-alist= variable needs further
configuration.  See [[LaTeX specific export settings]].

[fn:44] At the moment, some export backends do not obey this
...
[fn:45] Note that ~org-link-search-must-match-exact-headline~ is
locally bound to non-~nil~.  Therefore, ~org-link-search~ only matches
headlines and named elements.

[fn:46] Since commas separate the arguments, commas within arguments
have to be escaped with the backslash character.  So only those
backslash characters before a comma need escaping with another
backslash character.

[fn:47] If =BEAMER_ENV= is set, Org export adds =B_environment= tag
to make it visible.  The tag serves as a visual aid and has no
semantic relevance.

[fn:48] Please note that exported formulas are part of an HTML
...
[fn:49] This does not allow setting different bibliography compilers
for different files.  However, "smart" LaTeX compilation systems, such
as latexmk, can select the correct bibliography compiler.

[fn:50] Actually, the constructs =call_<name>()= and =src_<lang>{}=
are not evaluated when they appear in a keyword (see [[*Summary of
In-Buffer Settings]]).

[fn:51] In the following example, attempting to evaluate
...
[fn:52] Org Indent mode also sets ~wrap-prefix~ correctly for
...
[fn:53] This works, but requires extra effort.  Org Indent mode is
more convenient for most applications.

[fn:54] For a server to host files, consider using a WebDAV server,
such as [[https://nextcloud.com][Nextcloud]].  Additional help is at this [[https://orgmode.org/worg/org-faq.html#mobileorg_webdav][FAQ entry]].

[fn:55] While creating the agendas, Org mode forces =ID= properties
...
[fn:56] By default this works only for LaTeX, HTML, and Texinfo.
Configure the variable ~orgtbl-radio-table-templates~ to install
templates for other modes.

[fn:57] If the =TBLFM= keyword contains an odd number of dollar
